...
def dashboard():
    ...
    expenses = (
        ...
        .order_by(Transaction.date_of_expense.asc())
        .all()
    )
    ...
    amounts = [
        sum(expense.cost for expense in expenses if expense.category == category.name)
        ...
        for date in dates:
            if category.name in expenses_by_month_category[date]:
                data.append(
                    sum(
                        expense.cost
                        for expense in expenses_by_month_category[date][category.name]
                    )
                )
            else:
                data.append(0)
