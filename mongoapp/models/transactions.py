...
class Transaction(BaseModel):
    ...
    transactionType: str = Field(..., pattern="^(expense|income)$")  # Transaction type must be 'expense' or 'income'
    description: str
    group: str
    paidBy: str
    amount: float = Field(..., gt=0)  # Amount must be greater than 0
    currency: str
    split: Dict[str, float] = Field(..., min_items=1, )  # There must be at least one item in the split
    transactionId: str
    ...
    def check_transaction_id(cls, transaction_id, values):
        ...
        required_fields_present = date_present and paid_by_present and amount_present and group_present
        ...
        if required_fields_present:
            ...
            if transaction_id != expected_transaction_id:
                raise ValueError('Transaction ID must be a combination of transactionDate, paidBy, amount and group')
