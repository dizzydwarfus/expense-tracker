...
class MongoConnector(MyLogger):
    def __init__(self, connection_string):
        super().__init__(name='ExpenseTrackerWebApp', level='DEBUG', log_file='././logs.log')
...
class ExpenseTrackerWebAppDB(MongoConnector):
    ...
    def upsert_user(self, username, name, email, password):
        now = dt.datetime.now()
        user = User(name=name, email=email, password=hash_password(password), groups=[]).model_dump()
    ...
    def upsert_transaction(self, transaction_date: Union[str,dt.datetime], transaction_type: str, description: str, group: str, paidBy: str, amount: float, currency: str,  split: dict[str, float]):
        now = dt.datetime.now()
